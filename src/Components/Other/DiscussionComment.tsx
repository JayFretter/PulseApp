...
function DiscussionCommentBlock(props: DiscussionCommentProps) {
  return (
    <div className="bg-slate-800 pl-4 pt-4 pb-4 flex flex-col border-l-2 border-blue-300 gap-2 hover:bg-slate-600 transition-colors">
      ...
      {props.comment.children.map((comment, i) => {
        return <DiscussionCommentBlock comment={comment} key={i}/>;
      })}
    </div>
  );
}
